...
const flags = {
  ...
  tag: (argv.find((a) => a.startsWith("--tag=")) || "--tag=latest").slice(
    "--tag=".length,
  ),
};
...
function loadPackages() {
  ...
  for (const entry of readdirSync(PACKAGES_DIR, { withFileTypes: true })) {
    ...
    }).filter((d) => d.startsWith(SCOPE_PREFIX));
    out.push({
      name: pkg.name,
      version: pkg.version,
      dir: entry.name,
  ...
  return out.toSorted(
    (a, b) =>
      a.internalDeps.length - b.internalDeps.length ||
...
if (requested.length > 0) {
  ...
  const unknown = requested.filter(
    (r) => !packages.some((p) => p.name === r || p.dir === r),
  );
  if (unknown.length > 0)
    die(
      `Unknown package(s): ${unknown.join(", ")}\nKnown: ${packages.map((p) => p.name).join(", ")}`,
    );
  packages = packages.filter((p) => match(p));
}
...
for (const p of packages) {
  ...
  const status = already
    ...
    : c.green(`publish ${p.version}`) +
      (existing.length > 0
        ? c.dim(` (latest on npm: ${existing.at(-1)})`)
        : c.dim(" (new package)"));
...
if (toPublish.length === 0) {
  ...
  console.log(
    c.dim(
      "  pnpm --filter @standard-reader/renderer-core exec npm version patch --no-git-tag-version\n",
    ),
  );
  process.exit(0);
}
...
if (!flags.dryRun) {
  ...
  if (!whoami && !process.env.NPM_TOKEN) {
    die(
      "Not authenticated with npm. Run `npm login`, or set NPM_TOKEN in the environment (CI).",
    );
  }
  ...
  if (dirty) {
    console.log(
      c.yellow(
        "⚠ Working tree is dirty. Publishing packages built from uncommitted changes:",
      ),
    );
    console.log(
      c.dim(
...
for (const p of toPublish) {
  ...
  try {
    ...
    die(
      `Publish failed for ${p.name}@${p.version}. Already-published packages this run: ${published.map((x) => x.name).join(", ") || "none"}`,
    );
  }
}
