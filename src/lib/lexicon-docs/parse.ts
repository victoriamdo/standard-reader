...
export function parseLexiconDocument(raw: unknown): LexiconDocsEntry | null {
  ...
  return {
    ...
    primaryType: type,
    description:
      typeof main.description === "string" ? main.description : undefined,
    recordKey,
    fields,
    json,
  };
...
const SECTION_ORDER = new Map(
  LEXICON_DOCS_SECTIONS.map((section, index) => [section, index]),
);

export function sortLexiconDocsEntries(
  entries: Array<LexiconDocsEntry>,
): Array<LexiconDocsEntry> {
  return entries.toSorted((left, right) => {
    const sectionDelta =
      (SECTION_ORDER.get(left.section) ?? 0) -
      (SECTION_ORDER.get(right.section) ?? 0);
    if (sectionDelta !== 0) {
      return sectionDelta;
    }
