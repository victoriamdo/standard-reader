/* oxlint-disable typescript-eslint(ban-ts-comment) -- vendored third-party code */
// @ts-nocheck — vendored third-party code; colorjs.io's `number | null` channel
// types trip our strict null checks. Kept verbatim from upstream.
// Vendored from Radix UI (radix-ui/website: components/generate-radix-colors.tsx),
// whose generator is not published as a package. Generates 12-step Radix color
// scales (light or dark) from an accent + gray + background, so a collection theme
// yields a properly designed automatic dark mode.
...
function getButtonHoverColor(source: Color, scales: Array<ArrayOf12<Color>>) {
...
function getScaleFromColor(
  ...
  const allColors: Array<{ scale: string; color: Color; distance: number }> =
    [];
  ...
  const grayScaleNamesStr = grayScaleNames as ReadonlyArray<string>;
  ...
  const baseColor = [...scale].toSorted(
    (a, b) => source.deltaEOK(a) - source.deltaEOK(b),
  )[0];
