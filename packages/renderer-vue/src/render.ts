...
export interface RenderDocumentOptions {
  components?: VueComponentsInput;
  options?: RendererOptions;
}

/**
 * Render a Standard Site document to a Vue vnode (or `null` for an unsupported
 * or empty document). Use inside a render function or the `StandardDocument`
 * component.
 */
...
function runs(items: Array<RichText>, ctx: RenderContext): Array<Renderable> {
...
function renderBlock(node: BlockNode, ctx: RenderContext): Renderable {
  ...
  switch (node.type) {
    ...
    case "table": {
      const rows: Array<VueTableRow> = node.rows.map((row) =>
        row.map((cell) => ({
          header: cell.header,
